...
/**
 * REST controller for managing {@link com.gmo.permisfacilev2.domain.Question}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class QuestionResource {
        ...
        Optional<Question> result = questionRepository
            ...
            .map(existingQuestion -> {
                if (question.getIntitule() != null) {
                    existingQuestion.setIntitule(question.getIntitule());
                }
                if (question.getImage() != null) {
                    existingQuestion.setImage(question.getImage());
                }
                if (question.getDateCreation() != null) {
                    existingQuestion.setDateCreation(question.getDateCreation());
                }
                if (question.getLastUpdate() != null) {
                    existingQuestion.setLastUpdate(question.getLastUpdate());
                }
    ...
    /**
     * {@code GET  /questions} : get all the questions From.
     *@param lastIndex last question numero
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of questions in body.
     */
    @GetMapping("/questions/from/{lastIndex}")
    public List<Question> getAllQuestionsFrom(@PathVariable(value = "lastIndex", required = false) final Long lastIndex) {
        log.debug("REST request to get all Questions from lastIndex: {}", lastIndex);
        if(lastIndex == 0) {
            return questionRepository.findAll();
        }else {
            return questionRepository.findByNumeroGreaterThan(lastIndex);
        }
    }
