...
/**
 * REST controller for managing {@link com.gmo.permisfacilev2.domain.Reponse}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ReponseResource {
    ...
    public ResponseEntity<Reponse> partialUpdateReponse(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Reponse reponse
    ) throws URISyntaxException {
        ...
        Optional<Reponse> result = reponseRepository
            ...
            .map(existingReponse -> {
                if (reponse.getIntitule() != null) {
                    existingReponse.setIntitule(reponse.getIntitule());
                }
                if (reponse.getReponseUnique() != null) {
                    existingReponse.setReponseUnique(reponse.getReponseUnique());
                }
                if (reponse.getBonneReponse() != null) {
                    existingReponse.setBonneReponse(reponse.getBonneReponse());
                }
